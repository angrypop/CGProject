...
void ViewObject::Render(const ViewModule& module)
{
	...
	{
		if (module.CheckUniformName(data->GetName())) // this one is needed
		{
			data->Transfer(programHandle);
			countUniform++;
		}
	}
	if (countUniform != module.GetUniformNameNumber()) // not completed
	{
		throw("ViewObject::Render() Uniform Not Completed");
	}
	int index = 0;
...
void ViewTriangle::InitData(std::array<GLfloat, POINTSIZE * 3>&& vertices)
{
	...
	// add buffer data
	std::vector<GLsizei> bufferInfo = std::vector<GLsizei>{ COORDSIZE, TEXTURECOORDSIZE };
...
ViewPolygon::ViewPolygon()
	: ViewObject(ViewObjectEnum::PolygonObject, 0)
{
}
...
ViewRectangle::ViewRectangle(const GLfloat& width, const GLfloat& height)
	
{
	_type = ViewObjectEnum::RectangleObject;
	_verNum = 4;
	...
	this->ViewPolygon::InitData(vertices);
