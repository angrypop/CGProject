...
{
	this->Init(samplerName);


	::glBindTexture(GL_TEXTURE_2D, this->_BO);
	::glTexImage2D(GL_TEXTURE_2D, 0, internalformat, width, height, 0, format, GL_FLOAT, NULL);
	::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	::glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, this->_BO, 0);
...
void ViewTexture::Prepare(const GLuint& attachment)
{
...
void ViewTexture::Init(const std::string& samplerName)
{
...
void ViewRenderBuffer::Prepare(const GLuint& attachment)
{
	glActiveTexture(attachment);
	glBindTexture(GL_TEXTURE_2D, this->GetBO());
}
