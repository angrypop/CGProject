...
void GameWorld::InitGroups()
{
	...
	// GBuffer Render
	std::shared_ptr<MyGroup> myGroup = std::shared_ptr<MyGroup>(new MyGroup());
	for (const auto & i : GameObjects) {
		myGroup->AddGameObj(i);
	}
	myGroup->AddTestPolygon(100000);
	this->AddGroup(myGroup);

	//// GBuffer Illuminate
	//std::shared_ptr<GBufferIlluminateGroup> GBIGroup = std::shared_ptr<GBufferIlluminateGroup>(new GBufferIlluminateGroup());
	//this->AddGroup(GBIGroup);
