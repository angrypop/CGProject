...
//obj scene
GLfloat radius = 2;
glm::vec3 ObjPos(0.f, groundSize * 3 / 2, groundSize * 2);
glm::vec3 ObjVel(0, 0, 0);

//light scene
DirectionalLight directionalLight;

TexturedPlane plane[4];
FitTexturedPlane testFitPlane;
...
groundSize , groundSize * 3 / 2,  groundSize * 3, 1.0f , 1.0f };
	...
	testFitPlane = FitTexturedPlane(glm::vec3(0, 0, 0), planePoints5, sizeof(planePoints5), planeIndices3, sizeof(planeIndices3), planeShaders, sandTextures);
	testFitPlane.updateHeight(ObjPos, ObjVel, 0, 1, camera.GetViewMatrix(), projection);
...
groundSize , groundSize * 3 ,  groundSize * 3, 0.0f , 1.0f , 0.0f };
	...
	float temp = 1.5;
	for (int i = 0; i <= 20; i++) {
		float r = temp * temp + 0.3; r = r - floor(r); temp = sin(temp * temp) + 1.3;
		float g = temp * temp + 0.3; g = g - floor(g); temp = cos(temp * temp) + 1.3;
		float b = temp * temp + 0.3; b = b - floor(b); temp = sin(temp * temp) + 1.3;
		float a = temp * temp + 0.3; a = a - floor(a); temp = cos(temp * temp) + 1.3;
		testTransPlane.push_back(TransparentPlane(glm::vec3(0, i, 0), planePoints6, sizeof(planePoints6), planeIndices3, sizeof(planeIndices3), glm::vec4(r, g, b, a)));
	}
	skybox = SkyBox(1);
	...
	float e = 0.03;
	LightProjection = glm::ortho(SHADOW_WIDTH * -0.5f * e, SHADOW_WIDTH * 0.5f * e, 
	...
	//water 
	waterPlane = WaterPlane(-groundSize * 10, -groundSize * 10, groundSize * 10, groundSize * 10, groundSize, 100, 100);
}
...
void initOIT() {
	...
	glBufferData(GL_TEXTURE_BUFFER, 8 * width * height * sizeof(glm::vec4), NULL, GL_DYNAMIC_COPY);
	...
	OITPrograms[PlaneOIT] = glCreateProgram();
	ShaderInfo planeOITShaders[] = {
		//{GL_VERTEX_SHADER,"WaterOITV.shader"} ,
			{GL_VERTEX_SHADER,"gBufferTransPlaneV.shader"} ,
		{GL_FRAGMENT_SHADER,"PlaneOITF.shader"} ,
		{GL_NONE,""} };
	for (int i = 0; planeOITShaders[i].mode != GL_NONE; i++) {
		GLuint shader = LoadShader(planeOITShaders[i]);
		glAttachShader(OITPrograms[PlaneOIT], shader);
	}
	glLinkProgram(OITPrograms[PlaneOIT]);
