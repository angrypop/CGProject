...
static const float FOLLOW_SPEED = 0.02f; // must be in (0, 1)
...
public:
	...
	{
		float newPitch = asin(in_front.y) * RADIAN_TO_ANGLE;
		float newYaw = acos(in_front.x / sqrt(1 - in_front.y * in_front.y)) * RADIAN_TO_ANGLE;
		if (in_front.z < 0) newYaw *= -1;
		if (hard) {
			Pitch = newPitch;
			Yaw = newYaw;
		}
			...
			// smooth transition
			Pitch = (1 - FOLLOW_SPEED) * Pitch + FOLLOW_SPEED * newPitch;
			if (fabs(Yaw - newYaw) < 180)
				Yaw = (1 - FOLLOW_SPEED) * Yaw + FOLLOW_SPEED * newYaw;
			else // in case that yaw changes abruptly from +179 to -179 during a round turn
				Yaw = newYaw;
		}
		updateCameraVectors();
	}
