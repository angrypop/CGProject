...
void ViewWorld::Loop()
{
	constexpr double FPSsamplingRate = 5;
	...
	while (!glfwWindowShouldClose(_window)) {
		...
		if ((double)(end - start) / CLOCKS_PER_SEC >= FPSsamplingRate)
		{
			...
			// std::cout << Interaction::front[0] << " " << Interaction::front[1] << " " << Interaction::front[2] << std::endl;
		}
...
void GameWorld::InitGroups()
{
	...
	for (int i = 1; i <= 30; i++) {
		...
		GameObjects[i]->translate({ RandomReal(-10.0, 10.0), RandomReal(-10.0, 10.0), RandomReal(0.0, 10.0) });
	}
...
void GameWorld::UpdateCamera()
{
	...
	glm::vec3 cameraDelta = objFront * (-viewDist) +glm::vec3({ 0, 1, 0 });
	Interaction::camera.SetPosition(objPos + cameraDelta);
	if (!(Interaction::left_button_pressed || Interaction::right_button_pressed || Interaction::key_y_flag)) {
...
void GameWorld::UpdateAirplane()
{
	GLfloat deltaPower = 0.001f;
	GLfloat deltaYaw = 0.1f;
	...
	if (Interaction::key_space_pressed) {
		// reset
		airplane->setDir(airplane->getFrontDir(), {0, 0, 1});
		airplane->setDir(airplane->getUpDir(), {0, 1, 0});
		airplane->translate(-airplane->getPosition());
	}
	airplane->simulate();
}
