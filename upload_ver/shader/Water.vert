...
vec3 CalculateWavePosition(float q, float a, float w, vec3 dir, vec3 meshVert, float ph, float t) {
	float qa = q * a;
	float theta = dot(w * 1 * dir.xz, meshVert.xz) + ph * t * 4;
...
vec3 CalculateWaveNormal(float q, float a, float w, vec3 dir, vec3 waveVert, float ph, float t) {
	float wa = w * a;
	float theta = dot(w * 1 * dir, waveVert) + ph * t * 4;
	float cosTh = cos(theta);
	float sinTh = sin(theta);
...
void main() {
	...
	vec3 pos0 = CalculateWavePosition(q0, a0, w0, dir0, worldVertex, ph0, uniTime);
	vec3 pos1 = CalculateWavePosition(q1, a1, w1, dir1, worldVertex, ph1, uniTime);
	vec3 pos2 = CalculateWavePosition(q2, a2, w2, dir2, worldVertex, ph2, uniTime);
	vec3 pos3 = CalculateWavePosition(q3, a3, w3, dir3, worldVertex, ph3, uniTime);
	vec3 pos4 = CalculateWavePosition(q4, a4, w4, dir4, worldVertex, ph4, uniTime);
	vec3 pos5 = CalculateWavePosition(q5, a5, w5, dir5, worldVertex, ph5, uniTime);
	vec3 pos6 = CalculateWavePosition(q6, a6, w6, dir6, worldVertex, ph6, uniTime);
	vec3 pos7 = CalculateWavePosition(q7, a7, w7, dir7, worldVertex, ph7, uniTime);

	vec3 new_position = pos0 + pos1 + pos2 + pos3 + pos4 + pos5 + pos6 + pos7;
	new_position.xyz += worldVertex.xyz;

	vec3 nor0 = CalculateWaveNormal(q0, a0, w0, dir0, new_position, ph0, uniTime);
	vec3 nor1 = CalculateWaveNormal(q1, a1, w1, dir1, new_position, ph1, uniTime);
	vec3 nor2 = CalculateWaveNormal(q2, a2, w2, dir2, new_position, ph2, uniTime);
	vec3 nor3 = CalculateWaveNormal(q3, a3, w3, dir3, new_position, ph3, uniTime);
	vec3 nor4 = CalculateWaveNormal(q4, a4, w4, dir4, new_position, ph4, uniTime);
	vec3 nor5 = CalculateWaveNormal(q5, a5, w5, dir5, new_position, ph5, uniTime);
	vec3 nor6 = CalculateWaveNormal(q6, a6, w6, dir6, new_position, ph6, uniTime);
	vec3 nor7 = CalculateWaveNormal(q7, a7, w7, dir7, new_position, ph7, uniTime);
